...
class TransformsApp(Applet):
    ...
    async def update_transformed_images(self, dataset_ids, stateful=False):
    ...
    async def _update_images(self, dataset_ids, stateful=False):
        if stateful:
        ...
        await self.update_transformed_images(dataset_ids, stateful=stateful)

    async def _chunk_update_images(self, dataset_ids, stateful=False):
        ids = list(dataset_ids)

        for i in range(0, len(ids), UPDATE_IMAGES_CHUNK_SIZE):
            chunk = ids[i : i + UPDATE_IMAGES_CHUNK_SIZE]
            await self._update_images(chunk, stateful=stateful)

    async def _update_all_images(self, visible_images):
        with self.state:
            self.state.updating_images = True

        await self._chunk_update_images(visible_images, stateful=True)

        other_images = set(self.state.user_selected_ids) - set(visible_images)
        await self._chunk_update_images(other_images, stateful=False)
