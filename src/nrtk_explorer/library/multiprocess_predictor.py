...
class MultiprocessPredictor:
    def __init__(self, model_name="facebook/detr-resnet-50", force_cpu=False):
        ...
        self._response_thread = threading.Thread(target=self._process_responses, daemon=True)
    ...
    def _process_responses(self):
        while True:
            try:
                r_id, data = self._result_queue.get()
                print(f"Received response for {r_id}")
            except (EOFError, KeyboardInterrupt):
                break
            with self._lock:
                future = self._pending_futures.pop(r_id, None)
            if future and not future.done():
                # Use the stored event loop instead of getting a new one
                def set_future():
                    if not future.done():
                        future.set_result(data)

                asyncio.run_coroutine_threadsafe(self._schedule_set_future(set_future), self.loop)

    async def _schedule_set_future(self, callback):
        callback()
    ...
    async def _submit_request(self, command, payload):
        ...
        try:
            r = await future
            print(f"Got response for {req_id}")
            return r
